...
# RKH 11/11/19
def fitFlatTopPeakParams(B, C, E):  # Compose string argument for fit
    # print "name=EndErfc, B="+str(B)+", C="+str(C)
    return "name=FlatTopPeak, B=" + str(B) + ", C=" + str(C) + ", E=" + str(E)
...
def fitFlatTopPeak(fitPar, index, ws, outputWs):
    ...
    Fit(InputWorkspace=ws, Function=fitFlatTopPeakParams(centre, endGrad, width), StartX=str(start), EndX=str(end),
        Output=outputWs)
...
def getPoints(IntegratedWorkspace, funcForms, fitParams, whichTube, showPlot=False):
    ...
    for i in range(len(funcForms)):
        if funcForms[i] == 3:
            ...
            peakIndex = fitFlatTopPeak(fitParams, i, getPointsWs, calibPointWs)
...
def correctTubeToIdealTube(tubePoints, idealTubePoints, nDets, TestMode=False, polinFit=2):
    ...
    xResult = numpy.polynomial.polynomial.polyval(list(range(nDets)), c)
...
def getCalibratedPixelPositions_RKH(ws, tubePts, idealTubePts, whichTube, peakTestMode=False, polinFit=2):
    """     
...
def getCalibration_RKH(ws, tubeSet, calibTable, fitPar, iTube, peaksTable, meanCTable,
    ...
    for i in rangeList:
        ...
        if i in overridePeaks:
            ...
            # RKH add meanC
            actualTube, meanC = getPoints(ws, iTube.getFunctionalForms(), fitPar, wht, showPlot=plotThisTube)
            if plotThisTube:
        ...
        detIDList, detPosList = getCalibratedPixelPositions_RKH(ws, actualTube, iTube.getArray(), wht, peaksTestMode,
                                                                polinFit)
...
def getCalibrationFromPeakFile(ws, calibTable, iTube, PeakFile):
    ...
    for i in range(nTubes):
        ...
        detIDList, detPosList = getCalibratedPixelPositions_RKH(ws, actualTube, idealTube, wht)
